...
import com.zengine.graphics.OpenGL;
import com.zengine.graphics.Renderer;

import static org.lwjgl.opengl.GL11.*;
import static org.lwjgl.opengl.GL15.*;

public class TextBatch extends Renderer {
	
	public static final int RENDERER_MAX_SPRITES = 60000;
	public static final int RENDERER_VERTEX_SIZE = 44;
	...
	public TextBatch(Spritesheet spritesheet) {
		...
		OpenGL.vertexAttribPointer(2, 3, GL_FLOAT, false, RENDERER_VERTEX_SIZE, 5 * 4);
		OpenGL.vertexAttribPointer(3, 3, GL_FLOAT, false, RENDERER_VERTEX_SIZE, 8 * 4);
	...
	public void submit(Vector3f pos, Vector2f size, Vector4f texCoords, Vector3f foreground, Vector3f background) {
		data.putFloat(pos.x);
		data.putFloat(pos.y);
		data.putFloat(pos.z);
		data.putFloat(texCoords.x);
		data.putFloat(texCoords.w);
		data.putFloat(foreground.x);
		data.putFloat(foreground.y);
		data.putFloat(foreground.z);
		data.putFloat(background.x);
		data.putFloat(background.y);
		data.putFloat(background.z);
